...
# Example API-minimal VAT run using local FNIH VAT expression/metadata
# and LIGER program files.
#
# Input files used:
#   data/external/fnih/vat/norm_counts.tsv.gz
#   data/external/fnih/vat/sample_metadata.tsv.gz
#   cell_state_de/dat/vat/vat_cell_state_markers.gmt
#   cell_state_de/dat/api/curated_cell_state_manifest.tsv
#   data/external/liger/vat/<cell type>/gene_loadings.tsv
#   data/external/liger/vat/<cell type>/cell_scores.tsv, when present
#
# Final API-ready files written to results/fnih/vat/out:
...
#
# Intermediates are written only under results/fnih/vat/tmp:
...
#
# Default behavior uses CELL_SAMPLE_FRACTION=0.10 for a first-pass VAT run.
# Override examples:
#   TISSUE_ROOT=/path/to/vat bash cell_state_de/scripts/run_fnih_vat_api_data_pipeline.sh
#   TOP_PROGRAM_GENES=200 bash cell_state_de/scripts/run_fnih_vat_api_data_pipeline.sh
#   CELL_SAMPLE_FRACTION=1.0 bash cell_state_de/scripts/run_fnih_vat_api_data_pipeline.sh   # full run
#   CELL_SAMPLE_FRACTION=0.10 CELL_SAMPLE_SEED=1 bash cell_state_de/scripts/run_fnih_vat_api_data_pipeline.sh
#   PIGEAN_TRAIT_BLACKLIST=auto excludes HP_, exomes_, gcat_, and Orphanet traits.

PYTHON_CMD="${PYTHON_CMD:-../.venv/bin/python}"
TISSUE_ROOT="${TISSUE_ROOT:-results/fnih/vat}"
...
root = Path(os.environ.get('TISSUE_ROOT', 'results/fnih/vat'))
tmp = root / 'tmp'
tmp.mkdir(parents=True, exist_ok=True)
metadata = pd.read_csv('data/external/fnih/vat/sample_metadata.tsv.gz', sep='\t', compression='infer', low_memory=False)
...
TISSUE_ID="vat" \
EXPRESSION_TSV="data/external/fnih/vat/norm_counts.tsv.gz" \
EXPRESSION_VALUE_TYPE="${EXPRESSION_VALUE_TYPE}" \
METADATA="${TISSUE_ROOT}/tmp/input_metadata.tsv.gz" \
STATES_GMT="${CELL_STATE_DE_DIR}/dat/vat/vat_cell_state_markers.gmt" \
...
DATASET="scRNA" \
MODEL="mouse_msigdb" \
PROGRAM_ROOT="data/external/liger/vat" \
