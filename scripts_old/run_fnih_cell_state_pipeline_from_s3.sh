...
MANIFEST="${MANIFEST:-data/external/fnih/s3_locations.txt}"
S3_PREFIX="${S3_PREFIX:-s3://dig-analysis-data/single_cell}"
DATA_ROOT="${DATA_ROOT:-data/external/fnih}"
...
AWS_CMD="${AWS_CMD:-aws}"
QC_GMT="${QC_GMT:-cell_state_de/dat/qc/cmdkp_all_tissues_minimal_bad_cell_qc_signatures.gmt}"
STATE_MANIFEST="${STATE_MANIFEST:-cell_state_de/dat/api/curated_cell_state_manifest.tsv}"
...
read -r _manifest_first_line < "${MANIFEST}" || true
while read -r tissue s3_name _rest; do
  [[ -z "${tissue:-}" ]] && continue
  [[ "${tissue}" =~ ^# ]] && continue
  if [[ -n "${DATASET}" && "${tissue}" != "${DATASET}" ]]; then
    continue
  fi
  ...
  state_gmt="cell_state_de/dat/${dat_tissue}/${dat_tissue}_cell_state_markers.gmt"
  ...
  if [[ ! -s "${tissue_dir}/norm_counts.tsv.gz" || ! -s "${tissue_dir}/sample_metadata.tsv.gz" ]]; then
    log "Downloading ${tissue} from ${S3_PREFIX}/${s3_name}/"
    "${AWS_CMD}" s3 cp "${S3_PREFIX}/${s3_name}/" "${tissue_dir}/" --recursive \
...
PY
  ...
  if [[ ! -s "${rank_dir}/matrix.mtx.gz" ]]; then
    ...
    "${PYTHON_CMD}" cell_state_de/scripts/convert_expression_tsv_to_sparse_10x.py \
...
PY
  ...
  if [[ ! -s "${scoring_done}" ]]; then
    ...
    "${PYTHON_CMD}" cell_state_de/scripts/run_cmdkp_state_scoring.py \
  ...
  if [[ "${RUN_EXPRESSION}" == "1" && ! -s "${expression_done}" ]]; then
    ...
    "${PYTHON_CMD}" cell_state_de/scripts/summarize_state_expression.py \
