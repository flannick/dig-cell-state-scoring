...
#   data/external/liger/liver/<cell type>/gene_loadings.tsv
#   data/external/liger/liver/<cell type>/cell_scores.tsv, when present
#
# Final API-ready files written to results/fnih/liver/out:
...
root = Path(os.environ.get('TISSUE_ROOT', 'results/fnih/liver'))
tmp = root / 'tmp'
tmp.mkdir(parents=True, exist_ok=True)
metadata = pd.read_csv('data/external/fnih/liver/sample_metadata.tsv.gz', sep='\t', compression='infer', low_memory=False)
...
DATASET="scRNA" \
MODEL="mouse_msigdb" \
PROGRAM_ROOT="data/external/liger/liver" \
