...
# Example API-minimal artery run using local FNIH artery expression/metadata
# and LIGER program files.
#
# Input files used:
#   data/external/fnih/artery/norm_counts.tsv.gz
#   data/external/fnih/artery/sample_metadata.tsv.gz
#   cell_state_de/dat/artery/artery_cell_state_markers.gmt
#   cell_state_de/dat/api/curated_cell_state_manifest.tsv
#   data/external/liger/artery/<cell type>/gene_loadings.tsv
#   data/external/liger/artery/<cell type>/cell_scores.tsv, when present
#
# Final API-ready files written to results/fnih/artery/out:
...
#
# Intermediates are written only under results/fnih/artery/tmp:
...
#
# Default behavior uses CELL_SAMPLE_FRACTION=0.10 for a first-pass artery run.
# Override examples:
#   TISSUE_ROOT=/path/to/artery bash cell_state_de/scripts/run_fnih_artery_api_data_pipeline.sh
#   TOP_PROGRAM_GENES=200 bash cell_state_de/scripts/run_fnih_artery_api_data_pipeline.sh
#   CELL_SAMPLE_FRACTION=1.0 bash cell_state_de/scripts/run_fnih_artery_api_data_pipeline.sh   # full run
#   CELL_SAMPLE_FRACTION=0.10 CELL_SAMPLE_SEED=1 bash cell_state_de/scripts/run_fnih_artery_api_data_pipeline.sh

PYTHON_CMD="${PYTHON_CMD:-../.venv/bin/python}"
TISSUE_ROOT="${TISSUE_ROOT:-results/fnih/artery}"
...
root = Path(os.environ.get('TISSUE_ROOT', 'results/fnih/artery'))
tmp = root / 'tmp'
tmp.mkdir(parents=True, exist_ok=True)
metadata = pd.read_csv('data/external/fnih/artery/sample_metadata.tsv.gz', sep='	', compression='infer', low_memory=False)

cell_type_map = {
    'SMC': 'smc',
    'macrophage': 'macrophage',
    'mature NK T cell': 'mature_nk_t_cell',
    'pericyte': 'pericyte',
...
TISSUE_ID="artery" \
EXPRESSION_TSV="data/external/fnih/artery/norm_counts.tsv.gz" \
EXPRESSION_VALUE_TYPE="${EXPRESSION_VALUE_TYPE}" \
METADATA="${TISSUE_ROOT}/tmp/input_metadata.tsv.gz" \
STATES_GMT="${CELL_STATE_DE_DIR}/dat/artery/artery_cell_state_markers.gmt" \
...
DATASET="scRNA" \
MODEL="mouse_msigdb" \
PROGRAM_ROOT="data/external/liger/artery" \
